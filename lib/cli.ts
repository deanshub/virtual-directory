...
;(async function() {
  ...
  program
    ...
    .option(
      '-e, --exclusions <paths>',
      'list of exclusions from the virtual directory',
      ''
    )
  ...
  try {
    ...
    console.log(
      chalk.green(
        `virtual-directory linked directory to "${config.dest}", enjoy`
      )
    )
    if (program.cleanup) {
      setupCleanup(config)
    ...
    if (program.watch) {
      console.log(
        chalk.green(`Watch mode on${program.cleanup ? ' (with cleanup)' : ''}`)
      )
      watcher(config)
    }
  } catch (e) {
